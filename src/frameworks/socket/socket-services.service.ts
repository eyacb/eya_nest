...
import { AbstractWebSocketGateway } from "src/core/abstracts/socket-service.abstract";
...
export class NotificationsGateway extends AbstractWebSocketGateway {
  ...
  //         client.emit("newTransferRequest", request);
  //       } else if (
  //         clientInfo.role === "Artist" &&
  //         userIdString === recipientIdString
  //       ) {
  //         client.emit("transferRequestUpdated", request);
  //       }
  //     } catch (error) {
